...
from torch_geometric.nn import GATConv, InstanceNorm

class GAT_net(nn.Module):
    def __init__(self, dim, nhid, nlayers, nsamples, nbatch, radius, loss_fn, dim_emphasize, n_projections, heads=4):
        super(GAT_net, self).__init__()
        self.enc = nn.Linear(dim, nhid)
        self.convs = nn.ModuleList()
        for _ in range(nlayers):
            self.convs.append(GATConv(nhid, nhid // heads, heads=heads, concat=True))
